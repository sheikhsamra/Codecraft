...
export default function CategoryPage() {
  ...
  return (
    <div className="min-h-screen mx-auto max-w-[1600px] bg-slate-950 text-white px-6 py-16">
      <h2 className="text-6xl font-black mb-10 pb-2 leading-[1.1] tracking-tight bg-gradient-to-r from-fuchsia-400 via-white to-cyan-400 bg-clip-text text-transparent">Browse by Category</h2>
          ...
          {filteredBlogs.map((blog) => (
            <div key={blog._id} className="group bg-white/10 border border-white/10 rounded-3xl overflow-hidden shadow-xl hover:-translate-y-3 hover:bg-white/15 hover:shadow-[0_20px_50px_rgba(192,38,211,0.15)] transition-all duration-500">
              <div className="overflow-hidden">
                <img src={blog.image} alt={blog.title} className="h-56 w-full object-cover group-hover:scale-110 transition-transform duration-700" />
              </div>
              <div className="p-6">
                ...
                <Link to={`/blogs/${blog._id}`} className="block mt-6 text-center border border-fuchsia-500/30 text-fuchsia-400 py-3 rounded-xl font-bold hover:bg-gradient-to-r hover:from-fuchsia-600 hover:to-cyan-500 hover:text-white hover:border-transparent hover:shadow-lg hover:scale-105 transition-all duration-300">
                  Read More →
                </Link>
              </div>
            </div>
